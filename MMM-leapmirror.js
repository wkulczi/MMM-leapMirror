Module.register("MMM-leapmirror", {
  ...
  defaults: {
    ...
    actions: {
      ...
      leftHandActions: {
        isAnyFinger: true,
        finger0: {
          keyTap: "SPOTIFY_TOGGLE",
          screenTap: "ROSS_TOGGLE_VIDEO",
          circleClockwise: "SHOW_SCREEN",
          circleCounterclockwise: "HIDE_SCREEN",
          swipeUp: "SPOTIFY_NEXT",
          swipeDown: "SPOTIFY_PREVIOUS",
  ...
  start: function () {
    ...
    if (isAnyHand) {
      if (this.config.actions.leftHandActions) {
        this.config.actions.anyHandActions =
          this.config.actions.leftHandActions;
      } else {
        this.config.actions.anyHandActions =
          this.config.actions.rightHandActions;
      }
    }
  },
        ...
        this.controlller = Leap.loop((frame) => {
          ...
          if (!this.gestureLock) {
            if (frame.hands.length) {
              ...
              const hasAnyGestures = this.frameBuffer
                .map((value) => value.gestures.length)
                .some((gestureCounter) => gestureCounter > 0);
              if (hasAnyGestures) {
                this.parseGesture();
  ...
  useLeapGesture: function (frameGestureInfo) {
    ...
    console.log(
      `executinngAction -[${frameGestureInfo.direction}]: ${
        possibleActions[frameGestureInfo.direction]
      }`
    );
  ...
  parseGesture: function () {
    ...
    const dedupedGestures = LeapHelper.dedupContinuousMoves(
      this.frameBuffer.map((value) => value.gestures).flat()
    );
